...
%global giturl     https://github.com/NVieville
...
# Old stable release
# https://downloads.sourceforge.net/partimage/partimage-0.6.9.tar.bz2
# Last development sources available here:
# https://sourceforge.net/p/partimage/code/
# Download:
# https://sourceforge.net/code-snapshots/git/p/pa/partimage/code.git/partimage-code-e1762d096a810ca471913597f64602691be4e54b.zip
#
Source0:          %{giturl}/%{name}/archive/%{?gitcommit}%{!?gitcommit:%{version}}/%{name}-%{version}%{?gitshortcommit:-%{gitshortcommit}}.tar.gz
...
%prep
%setup -n %{name}-%{?gitcommit}%{!?gitcommit:%{version}} -q -a1 -a2
%{__rm} -rf ./m4/*.m4
./autogen.sh
...
%{__cat} <<EOF >partimage-server.logrotate
...
%{__install} -Dp -m0644 %{SOURCE4} %{buildroot}%{_unitdir}/partimaged.service
%{__install} -Dp -m0644 partimage-server.logrotate %{buildroot}%{_sysconfdir}/logrotate.d/partimage-server
...
%doc AUTHORS BUGS ChangeLog FORMAT README* THANKS
...
%config(noreplace) %{_sysconfdir}/logrotate.d/partimage-server
...
* Tue Dec 02 2008 Paulo Roma <roma@lcg.ufrj.br> - 0.6.7-5
...
* Thu Jul 31 2003 Dag Wieers <dag@wieers.com> - 0.6.2-1
